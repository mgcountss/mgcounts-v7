...
let id = "google-inc-fire-susan-wojcicki-as-youtube-ceo";
let last = []
if (url.searchParams.get('id')) {
    id = params.get("id");
}
function load() {
    document.getElementById('embed').value = "https://mgcounts.com/embeds/changeorg?id="+id+""
...
let lastest = 0;
function fetcher() {
    fetch('https://backend.mgcounts.com/changeorg/' + id + '')
        .then(response => response.json())
        .then(data => {
            document.getElementById('count').innerHTML = data.main
            lastest = parseFloat(data.main)
            document.getElementById('name').innerHTML = data.name
            document.getElementById('img').src = "../assets/img/change.png"
            ...
            if (data.banner == null) {
                document.getElementById('banner').src = "../assets/img/change.png"
            } else {
                document.getElementById('banner').src = "../assets/img/change.png"
            }
            console.log(data.sidecounts[0].count)
            document.getElementById('count_1').innerHTML = (parseFloat(data.sidecounts[0].count) - parseFloat(data.main))
            document.getElementById('name_1').innerHTML = data.sidecounts[0].name
        });
}
