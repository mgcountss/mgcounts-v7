...
let id = "huaPiB3qL1aE7gMCsWJxu9JLiLF3";
let last = []
if (url.searchParams.get('id')) {
    id = params.get("id");
}
function load() {
    document.getElementById('embed').value = "https://mgcounts.com/embeds/storyfire/user?id="+id+""
...
let lastest = 0;
function fetcher() {
    fetch('https://backend.mgcounts.com/storyfire/user/' + id + '')
...
function search() {
    fetch('https://backend.mgcounts.com/storyfire/users/search/' + document.getElementById('search').value + '')
    .then(response => response.json())
    .then(data => {
