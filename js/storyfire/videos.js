...
let id = "61d75540c6a65999bf404946";
let last = []
if (url.searchParams.get('id')) {
    id = params.get("id");
}
function load() {
    document.getElementById('embed').value = "https://mgcounts.com/embeds/storyfire/videos?id=" + id + ""
...
let lastest = 0;
function fetcher() {
    fetch('https://backend.mgcounts.com/storyfire/videos/' + id + '')
...
function search() {
    fetch('https://backend.mgcounts.com/storyfire/videos/search/' + document.getElementById('search').value + '')
        .then(response => response.json())
        .then(data => {
