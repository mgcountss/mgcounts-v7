...
function load() {
    document.getElementById('embed').value = "https://mgcounts.com/embeds/twitch/stream?id="+id+""
...
let lastest = 0;
function fetcher() {
    fetch('https://backend.mgcounts.com/twitch/streams/' + id + '')
        .then(response => response.json())
        .then(data => {
            if (data.stream === null) {
                document.getElementById('name').innerHTML = "Offline Stream"
                document.getElementById('img').src = "https://external-content.duckduckgo.com/iu/?u=https%3A%2F%2Fyt3.ggpht.com%2Fa-%2FAAuE7mBRP_TlZhmY3ddIFRhwViwOAgo-69QVjdLx%3Ds900-mo-c-c0xffffffff-rj-k-no&f=1&nofb=1"
                document.getElementById('banner').src = "https://external-content.duckduckgo.com/iu/?u=https%3A%2F%2Ftse3.mm.bing.net%2Fth%3Fid%3DOIP.F1emXleAI6upuEb7wYxBLgHaEK%26pid%3DApi&f=1"
            } else {
...
function search() {
    fetch('https://backend.mgcounts.com/twitch/users/search/' + document.getElementById('search').value + '')
    .then(response => response.json())
    .then(data => {
